...
static void printStyleAttributeForNode(id node)
{
    ...
    for (int i=0; i<[children count]; i++) {
        ...
        printStyleAttributeForNode(elt);
    }
}

@implementation Definitions(fmekwlfmklsdmfklmsdklfmklsfjkdsjkfjfdksfjfjdsk)
+ (void)printStyleAttribute
{
    id data = [Definitions dataFromStandardInput];
    id str = [data asString];
    id document = [str parseHTML];
    printStyleAttributeForNode(document);
}
+ (void)printStyleAttribute:(id)url
{
