...
@implementation Definitions(fmeklwmfklsdklfmklsdmfkl)
...
+ (void)MailInterface
{
    ...
    arr = [arr keepSuffix:@".eml"];
    if (![arr count]) {
        obj = [nsarr() asMailInterface];
    } else {
        id cmd = nsarr();
        [cmd addObject:@"remoulade-list"];
        [cmd addObjectsFromArray:arr];
        id output = [cmd runCommandAndReturnOutput];
        id results = [output blocksFromData];
        obj = [results asMailInterface];
    }
    if (obj) {
...
+ (id)MailInterface:(id)path
{
    id arr = [path contentsOfDirectoryWithFullPaths];
    arr = [arr keepSuffix:@".eml"];
    if (![arr count]) {
        return [nsarr() asMailInterface];
    }
    ...
    id output = [cmd runCommandAndReturnOutput];
    id results = [output blocksFromData];
    id obj = [results asMailInterface];
    return obj;
}
@end
...
- (id)asMailInterface
{
    ...
    [obj setValue:@"MailHTMLMessageHeaders:(NSArray|addObject:'remoulade-printHeaders'|addObject:(selectedObject|name)|runCommandAndReturnOutput|asString) plain:(NSArray|addObject:'remoulade-printContentType'|addObject:(selectedObject|name)|addObject:'text/plain'|runCommandAndReturnOutput|asString) html:(NSArray|addObject:'remoulade-printContentType'|addObject:(selectedObject|name)|addObject:'text/html'|runCommandAndReturnOutput|asString) :(NSArray|addObject:'remoulade-extractPart'|addObject:(selectedObject|name)|runCommandAndReturnOutput|asString|lines)" forKey:@"defaultMessageForClick"];
    return obj;
}
@end
